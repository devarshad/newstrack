...
namespace NewsTrack.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        //
        // GET: /Account/RequireEmailConfirmation
        [AllowAnonymous]
        public ActionResult RequireEmailConfirmation()
        {
            return View();
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        await this.UserManager.AddToRoleAsync(user.Id, "Visitor");
                        // Send an email with this link
                        string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);

                        Logger.Debug("After generating token");

                        var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                        await UserManager.SendEmailAsync(user.Id, "Confirm your account", "Please confirm your account by clicking <a href=\"" + callbackUrl + "\">here</a>");

                        Logger.Debug("After email sent");
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            ...
            {
                User _user = new User();
                var user = _user.FindUser(model.UserKey);
                //var user = await UserManager.FindByNameAsync(model.Email);
                if (user == null)
                {
                    ViewBag.Message = "This user does not exist";
                    ModelState.AddModelError("notfound", "This user does not exist");
                }
                else if (!(await UserManager.IsEmailConfirmedAsync(user.Id)))
                {
                    ViewBag.Message = "This user's email has not verified yet.";
                    ModelState.AddModelError("notverified", "This user's email has not verified yet.");
                }
                else
                {
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            if (user == null)
            {
                ...
                ViewBag.Message = "This user name does not exist";
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                default:
                    ...
                    return await ExternalLoginConfirmation(
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                        UserManager.ConfirmEmail(user.Id);
                        return RedirectToLocal(returnUrl);
                    }
                }
                AddErrors(result);
        ...
        private bool ValidateCaptcha(string response)
        {
            ...
            const string secret = "6LfoXQ8TAAAAAP4ulndUE4w7Vpaa3VaA73eCKpsj";
            ...
            if (!captchaResponse.Success)
            {
                ...
                switch (error)
                {
                    ...
                    case ("missing-input-response"):
                        ViewBag.CaptchaMessage = "The response parameter is missing.";
                        break;
                    case ("invalid-input-response"):
                        ViewBag.CaptchaMessage = "The response parameter is invalid or malformed.";
                        break;
